...
export async function listTasks(
    ...
    const [sql, sqlParams] = getQuerySql(userId, params);
...
export async function createTask (task: {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
        `
        INSERT INTO tasks (title, description, start_date, due_date, priority, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)
        `,
...
export async function updateTaskStatus(
    ...
    const [result] = await pool.execute<ResultSetHeader>(
        `
        UPDATE tasks SET status = ?, update_time = CURRENT_TIMESTAMP
...
export async function findTaskByIdForUser(taskId: number, userId: number): Promise<TaskRecord | null> {
